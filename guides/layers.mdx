---
title: "Rendering Layers for Popups, Modals, and Tooltips"
sidebarTitle: "Layers"
description: "Control draw order and input priority with Paper's integer layer system. Learn how Base, Overlay, and Topmost layers work, and how to build popups and modals."
---
...
```csharp
bool isOpen = paper.GetElementStorage<bool>(dropdownId, "open");

var trigger = paper.Box("DropdownTrigger")
    .Width(160).Height(36)
    ...
    .OnClick(_ =>
    {
        bool current = paper.GetElementStorage<bool>(dropdownId, "open");
        paper.SetElementStorage(dropdownId, "open", !current);
    });
        ...
        foreach (var option in dropdownOptions)
        {
            paper.Box("Option", option.Index)
                ...
                .OnClick(_ =>
                {
                    ...
                    paper.SetElementStorage(dropdownId, "open", false);
                });
        }
    }
}
