...
```csharp
// From Samples/OpenTK/Program.cs — inside PaperTKWindow
// (renderer is instantiated in OnLoad, then passed to the Paper constructor)
...
Paper uses `FontAtlasSettings` for atlas configuration and `Canvas.AddFallbackFont` to register typefaces. Load a font file before your frame loop starts:

```csharp
// Load a TTF font file and register it as the fallback font
...
Paper.Box("AnimatedButton")
    ...
    .Active
        ...
        .End()
    .Transition(GuiProp.BackgroundColor, 0.2, Paper.Easing.SineInOut)
    .Text(Text.Center("Click Me", myFont, Color.White))
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub — Samples" icon="github" href="https://github.com/ProwlEngine/Prowl.Paper/tree/main/Samples">
    Browse the complete OpenTK, Raylib, and WebAssembly sample projects to see a fully wired-up application.
  </Card>
</CardGroup>
